...
function buildDeptKPIs(
  ...
  return DEPARTMENT_KPIS.map(kpiDef => {
    ...
    // Staleness detection with 7-day cutoff
    ...
    if (isStale && deptLatestDate) {
      const staleDays = (new Date(latestDate).getTime() - new Date(deptLatestDate).getTime()) / (1000 * 60 * 60 * 24);
    ...
    const secondaryKpis = secDefs.map(secDef => {
      const latestFields2 = effectiveFields;
    ...
    // lastSubmissionDate reuses deptLatestDate (computed above)
    ...
    return {
      ...
      secondaryKpis,
      health,
      lastSubmissionDate: deptLatestDate,
...
export async function GET(req: NextRequest) {
  ...
  // --- Admissions MTD from admissions_log (IP admissions only) ---
  try {
    if (current) {
      const admResult = await sql`
        SELECT COUNT(*) as total FROM admissions_log
        WHERE admission_date >= ${currentMonth + '-01'}::date
          AND admission_date < (${currentMonth + '-01'}::date + interval '1 month')
          AND admission_no LIKE 'IP-%'
      `;
      const admCount = parseInt(admResult.rows[0]?.total || '0', 10);
      if (admCount > 0) {
        current.totalAdmissions = admCount;
      }
    }
    if (previous) {
      const prevAdmResult = await sql`
        SELECT COUNT(*) as total FROM admissions_log
        WHERE admission_date >= ${prevMonth + '-01'}::date
          AND admission_date < (${prevMonth + '-01'}::date + interval '1 month')
          AND admission_no LIKE 'IP-%'
      `;
      const prevAdmCount = parseInt(prevAdmResult.rows[0]?.total || '0', 10);
      if (prevAdmCount > 0) {
        previous.totalAdmissions = prevAdmCount;
      }
    }
  } catch (e) {
    console.error('Admissions log query skipped:', e);
  }
