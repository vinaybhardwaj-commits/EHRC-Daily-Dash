...
import { upsertDepartmentData, saveHuddleSummary } from '@/lib/storage';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const uploadType = formData.get('type') as string; // 'department-data' or 'huddle-summary'
    ...
    if (uploadType === 'huddle-summary') {
      ...
      // Save huddle summary to Postgres
      await saveHuddleSummary(targetDate, {
        filename: file.name,
        content,
