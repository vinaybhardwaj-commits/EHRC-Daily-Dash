...
export default function ExecutiveSummary({ snapshot }: Props) {
  ...
  // Merge both stored entry shapes per dept so web-form {key,value} days aren't blank.
  const fieldsBySlug: Record<string, Record<string, string | number>> = {};
  for (const d of snapshot.departments) fieldsBySlug[d.slug] = mergeEntryFields(d.entries);
  ...
  const otCases = extractNumeric(getDeptField('ot', 'Total cases done today'));
  ...
  if (revenue !== null) metrics.push({ label: 'Revenue Today', value: `₹${formatCurrency(revenue, true)}`, icon: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z', color: 'emerald' });
  if (revenueMTD !== null) metrics.push({ label: 'Revenue MTD', value: `₹${formatCurrency(revenueMTD, true)}`, icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', color: 'emerald' });
  ...
  if (arpob !== null) metrics.push({ label: 'ARPOB', value: `₹${formatCurrency(arpob, true)}`, icon: 'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6', color: 'teal' });
  ...
  if (pharmacyMTD !== null) metrics.push({ label: 'Pharmacy MTD', value: `₹${formatCurrency(pharmacyMTD, true)}`, icon: 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z', color: 'emerald' });
